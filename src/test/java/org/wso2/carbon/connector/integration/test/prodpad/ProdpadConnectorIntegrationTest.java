...
public class ProdpadConnectorIntegrationTest extends ConnectorIntegrationTestBase {
    ...
    public void testcreateIdeaWithOptionalParameters() throws IOException, JSONException {
        ...
        Assert.assertEquals(esbRestResponse.getHttpStatusCode(), 201);
        final String ideaId = esbRestResponse.getBody().getString("id");
        final String accountId = esbRestResponse.getBody().getJSONObject("account").getString("id");
        final String creatorId = esbRestResponse.getBody().getJSONObject("creator").getString("id");
        connectorProperties.setProperty("accountId", accountId);
        connectorProperties.setProperty("ideaId", ideaId);
        connectorProperties.setProperty("creatorId", creatorId);

        final String apiEndpoint = apiEndpointUrl + "/ideas/" + ideaId + "?apikey=" + apiKey;
        RestResponse<JSONObject> apiRestResponse = sendJsonRestRequest(apiEndpoint, "GET", apiRequestHeadersMap);

        Assert.assertEquals(apiRestResponse.getBody().getString("title"), esbRestResponse.getBody().getString("title"));
        Assert.assertEquals(apiRestResponse.getBody().getString("id"), connectorProperties.getProperty("ideaId"));
        Assert.assertEquals(apiRestResponse.getBody().getJSONObject("account").getString("id"),
                connectorProperties.getProperty("accountId"));
        Assert.assertEquals(apiRestResponse.getBody().getJSONObject("creator").getString("id"),
                connectorProperties.getProperty("creatorId"));
    }
    ...
    public void testCreateIdeaWithNegativeCase() throws IOException, JSONException {
        ...
        Assert.assertEquals(esbRestResponse.getBody().getString("reason"), apiRestResponse.getBody()
                .getString("reason"));
    }
    ...
    public void testListIdeasWithOptionalParameters() throws IOException, JSONException, InterruptedException {
        ...
        final String apiEndPoint =
                apiEndpointUrl + "/ideas?apikey=" + apiKey + "&page=1&product=" + product
    ...
    public void testUpdateIdeaStatusWithOptionalParameters() throws IOException, JSONException {
        ...
        Assert.assertEquals(commentObj.getString("comment"),
                esbRestResponse.getBody().getJSONObject("comments").getString("comment"));
     ...
     * @throws JSONException
     */
    @Test(groups = { "wso2.esb" }, dependsOnMethods = { "testcreateIdeaWithOptionalParameters", "testListUserStoriesWithMandatoryParameters" }, description = "prodpad {getUserStory} integration test with mandatory parameters.")
    ...
    public void testListUserStoriesWithMandatoryParameters() throws IOException, JSONException {
        ...
        connectorProperties.put("userStoryId" , apiUserStoryArray.getJSONObject(0).getString("id"));
